...
Meteor.startup(function() {
  accountConfig(); // 账号配置初始化
  supportInfoInit();   // 相关辅助信息初始化
  productInit();  // 产品初始化
});
...
// 系统辅助信息初始化
function supportInfoInit() {
  var retInfo = getSupportInfo();
  var versionInfo = retInfo.version;
  var versionNow = versionInfo.versions;
  var collMap = retInfo.collMap || {
    'Business': Business,
    'Business1': Business1,
    'BusinessTypeLists': BusinessTypeLists,
    'BankDes': BankDes,
  };

  var business = retInfo.business;
  var business1 = retInfo.business1;

  var supportInfoVerSaved = WebSiteInfoMobile.findOne({name: 'supportInfoVersion'});
  if (!supportInfoVerSaved) {
    supportInfoVerSaved = {
      name: 'supportInfoVersion',
      versions: {
        'BusinessTypeLists': 0,
        'Business': 0,
        'Business1': 0,
    ...
    WebSiteInfoMobile.insert(supportInfoVerSaved);
  }
  var versionSaved = supportInfoVerSaved.versions;

  for (var collName in versionNow) {
    if ( (!versionSaved[collName]) || (versionSaved[collName] < versionNow[collName]) ) {
      var collInfo = retInfo[collName] || [];
      var coll = collMap[collName];
      coll.remove({});
      collInfo.forEach(function (info) {
        coll.insert(info);
      });
    }
  }
  WebSiteInfoMobile.update({name: 'supportInfoVersion'}, versionInfo);
}
...
function getSupportInfo() {
  return {
    ...
    BankDes: [
      {name: '建设银行虹桥路支行',  payment: '350元左右', star: 5, des: '无需本人到场，需身份证原件1次'},
      {name: '工商银行昌化路支行',  payment: '首年年费750元', star: 4, des: '无需本人到场，需身份证原件1次，需要查看实际场地'},
      {name: '中国银行蒙自路支行',  payment: '首年免年费', star: 4, des: '需身份证原件及电话、网银管理员身份证原件及电话；开通网银需法人亲自办理'},
      {name: '上海银行瑞虹新城支行',  payment: '首年免年费', star: 4, des: '需法人或者代表人2次到场'},
      {name: '上海银行天宝支行',  payment: '首年免年费', star: 4, des: '需法人或者代表人2次到场'},
      {name: '招商银行长乐支行',  payment: '首年600元', star: 4, des: '需身份证原件，需派人到场一次做现场确认，存款5k-1W（投资款）'},
      {name: '建设银行凤阳路支行',  payment: '350元左右', star: 3, des: '需法人或者代表人2次到场'},
      {name: '建设银行南京西路支行',  payment: '350元左右', star: 3, des: '需法人或者代表人2次到场'},
    ],
  }
}
...
function productInit() {
  ...
  if (productVerSaved.CompanyRegist <= productVersion.CompanyRegist) {
    CompanyRegist.remove({});
    companyRegist.forEach(function (info) {
      CompanyRegist.insert(info);
    });
  }

  if (productVerSaved.FinanceAgent <= productVersion.FinanceAgent) {
    FinanceAgent.remove({});
    financeAgent.forEach(function (info) {
      FinanceAgent.insert(info);
    });
  }

  if (productVerSaved.SpecialProduct <= productVersion.SpecialProduct) {
    SpecialProduct.remove({});
    specialProduct.forEach(function (info) {
      SpecialProduct.insert(info);
    });
  }

  if (productVerSaved.BankLists <= productVersion.BankLists) {
    BankLists.remove({});
  ...
  WebSiteInfoMobile.update({name: 'productVersion'}, {$set: productVersion});
}
